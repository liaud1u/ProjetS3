        ...
        if (SDL_PollEvent(&event)){
            ...
            if (keystate[SDLK_q] ){
                ...
                for(ib=0;ib<i;ib++){
                    tilePosition.y = 55 - ho + D_SIZE * jb;
                    tilePosition.x = 75 + ve + D_SIZE * ib;
                    if(tilePosition.x -16 -50<= LARGEUR/2 && tilePosition.x +50-50 -16 >= LARGEUR/2 && tilePosition.y <= HAUTEUR/2 && tilePosition.y + 50 >= HAUTEUR /2){
                        actualX = jb;
                        actualY = ib;
                      }
                  }
            }
                if(mapix[actualX][actualY-1]==49 ||  mapix[actualX][actualY]==50)
                {ve +=2;}
